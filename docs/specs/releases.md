...
`test/integration/install.test.ts` covers what `verify` structurally cannot: rejected downloads, upgrades, the platform-specific restart advice, and the build-to-installer asset mapping before a tag is spent. It stubs `curl`, `uname` and `mv` through `PATH`, seams the installer already has, so either platform is reachable from either host and the preconditions for a single-filesystem rename are checked directly.
