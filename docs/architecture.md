...
Evidence is per cycle, not carried forward. A poll that fails claims nothing at all — keeping the previous cycle's answer would stop an outage promoting work judged ineligible, but would still let work last seen as eligible start long after that observation stopped being current, and those are the same mistake in opposite directions. At one review per twenty minutes, waiting for GitHub to answer again costs nothing; starting on stale evidence does. This also subsumes any "has this process ever polled" rule: a restart simply has no successful cycle yet.
...
**Shutdown does not start work.** `runLoop` re-checks the abort signal after polling and again before sleeping, because a poll takes seconds and the review it would start takes twenty minutes. Without that, `service install` — which stops the old service first — could begin a review launchd is already counting down to SIGKILL.
