...
3. **A review is governed by your Claude Code configuration, never the branch under review.** `claude --setting-sources user`. The behaviour was established and re-checked by experiment; `doctor` checks only that the flag is still validated, not that its meaning is unchanged.
...
**A review request is identified by its GitHub issue event, not by `(repo, pull, sha)`.** `requested_reviewers` is state — GitHub clears you from it the moment you submit a review — so it cannot express "they asked again." The `review_requested` issue events can, each with its own id. Identifying a request by revision would swallow a genuine second ask, and a second ask is not a retry.

**Identity comes from the issue events, not the timeline.** The timeline is a superset that interleaves every commit, comment, review and cross-reference, and discovery reads it once per candidate on every poll — for `oven-sh/bun#30412`, 6,049 KB across 17 API pages; the issue events returned 375 KB across 4. Measured across eight pull requests in two repositories, from 9 to 1,663 timeline entries: the two endpoints agreed on every field Engwire reads, plus `node_id` and `url`, for all nine `review_requested` entries between them — five naming a user, four a team ([experiments.md](experiments.md)). That `node_id` and `url` also match is what makes these two projections of one object rather than two records that agree today; the `id` matching is what makes the switch free, since `UNIQUE(event_id)` keeps matching against a database an earlier Engwire wrote and nothing is reviewed a second time for having changed endpoints.

**Discovery reads REST, not GraphQL.** GraphQL could batch many candidates into one request where discovery now spends two `gh` subprocesses on each. But `ReviewRequestedEvent` carries no `databaseId`, and the REST integer is the identity everything downstream is built on — `UNIQUE(event_id)`, `BigInt` ordering, `CAST(event_id AS INTEGER)`. So this is not a smaller change but a second identity scheme, with a cutover to design for requests already recorded. Not worth that against subprocess spawns on a laptop that polls once a minute; revisit if the spawn count ever hurts.

**The revision comes from the pull request, not the event.** GitHub leaves `commit_id` null on `review_requested` events — verified against live pull requests, not assumed — so there is no revision on the event to read. Every successful poll refreshes a queued run from the pull request's current head. Claiming snapshots the latest head observed for that attempt; a claim released before the agent starts returns to the live queue, where a later poll may refresh it again. That is also the revision the reviewer would see on opening the pull request at that poll.
...
Whether the flag covers *all* of that was tested rather than assumed; [experiments.md](experiments.md) records the setup, results and Claude Code versions. Since this is CLI behaviour rather than Engwire's, `doctor` runs Engwire's invocation and confirms that an invalid setting source is refused. That checks the interface, not its semantics: a flag still validated but no longer applied would pass. There is deliberately no opt-out for trusted repositories.
...
Claude can exit 0 without running a review when a skill is missing, non-invocable, reserved or disabled. Without a preflight, Engwire would record that invocation as `completed` and consume an event GitHub will not send again. [experiments.md](experiments.md) records the observed output and versions.

The preflight checks only what Engwire can establish from the named skill: its folder name, a readable `SKILL.md`, and whether its front matter leaves it invocable. It accepts the measured values `true`, `1`, `yes` and `on`, with measured surrounding whitespace removed; other values and duplicate declarations fail closed. Claude accepts more spellings, but generalising beyond the tested set risks spending a request if one stops working. `skillOverrides` remains outside the check because interpreting Claude's settings would duplicate another product's configuration model. Engwire does not infer failure from an empty transcript: a skill may post through a tool and then say nothing, while the measured silent failures are already rejected by the preflight.
...
**A draft is held, not dismissed — the one request deliberately left unrecorded.** Marking a draft ready does not re-request the reviewers it already had. Measured against live pull requests: of ten reviewers requested while a pull request was a draft, eight received no fresh `review_requested` event when it became ready — the events that do fire are for code owners and newly added reviewers. Recording a dismissal would therefore consume the only event Engwire is ever going to see, and the review would be dropped in silence, which is the worst failure this product has. Leaving it unrecorded only repeats the event read discovery already performs each poll; the request waits until the pull request is ready, is withdrawn, or closes. A held request still supersedes an older queued run for the same pull request — that run is answering a question this one replaced, and letting it proceed would review the pull request now and again once it is ready.
...
**Polling sees what is still being asked when it looks.** Discovery starts from the pull requests that currently request the reviewer, then reads their issue events. A request made and withdrawn — or answered by someone else — entirely between two polls is never observed at all. Exact event capture would need a webhook, and therefore a relay with access to the code, which is the one thing this design exists to avoid.
